...
export default class Triangle{
    ...
    aux: any;
    static clipPlane(planePoint: Vector3D, planeNormal: Vector3D, tri: Triangle, any=null): Triangle[]{ //return liste triangle, vide, tri de base, 1 ou 2 nouveaux triangles
        planeNormal = Vector3D.normalise(planeNormal); //important normal
    
        //plus petite distance point, plan, SIGNÉ (+,-)
        function distance(p: Vector3D): number{
            return Vector3D.dotProduct(planeNormal, p) - Vector3D.dotProduct(planeNormal, planePoint);
        }
    
        let insidePoints: Vector3D[] = []; //derriere le plan (à afficher)
        let outsidePoints: Vector3D[] = []; //avant le plan (derriere le joueur)
        let insideTextures: Vector2D[] = [];
        let outsideTextures: Vector2D[] = [];
        ...
        if( nombreInsidePoints == 1 && nombreOutsidePoints == 2){ //2 points derriere, a remplacer
            let tri1: Triangle = tri.copy(); //permet de choisir entre triangle et triangle of block
    ...
    copy(): Triangle{
        let tmp = new Triangle( [this.p[0].copy(), this.p[1].copy(), this.p[2].copy()],
                                [this.t[0].copy(), this.t[1].copy(), this.t[2].copy()]);
